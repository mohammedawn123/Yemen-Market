...
@section('content')
    ...
    <div class="row">
       ...
       @if($cart['items'])
                        ...
                        <tr >
                            ...
                            <td  class="text-left">
                                <div class="input-group btn-block" style="max-width: 250px;">
                                    <input type="text" name="quantity" value="{{ $list['qty']}}" size="1" class="form-control qty" data-id="{{$list['id']}}">
                                    <span class="input-group-btn">
                                    <button type="button" id="button" data-id="{{$list['id']}}" data-instance="default" onclick="update($(this));" data-toggle="tooltip" title="" class="btn btn-primary addToCart" data-original-title="Update"><i class="fa fa-refresh"></i></button>
                                    <a href="{{route('item.remove' , ['id'=> $list['id'] , 'instance'=>'default'])}}" data-toggle="tooltip" title="" class="btn btn-danger"   data-original-title="Remove"><i class="fa fa-times-circle"></i></a>
                               </span>
                                </div>
                            </td>
                            <td class="text-left">  {{currency_symbol($list['price'])}}</td>
                            <td class="text-left" >   {{currency_symbol($list['price'] * $list['qty'])}}</td>
        ...
        <div class="col-md-12">
            <form class="sc-shipping-address" id="form-process" role="form" method="get" action="{{route('cart.list')}}">
                @csrf
                <div class="row">
                    <div class="col-md-6">
