  ...
  @section('content')
      ...
      <div class="container-fluid">
          ...
          <div class="panel panel-default">
              <div class="panel-heading">
                  <h3 class="panel-title"><i class="fa fa-list"></i> {{'Language Form'}} </h3>
              </div>
              <div class="panel-body">
                      ...
                      <div class="tab-content">
                          <div class="tab-pane active" id="tab-general">
                              <div class="form-group required">
                                  ...
                                  <div class="col-sm-10">
                                      <input type="text" name="name" value="{{isset($name) ? $name : ''}}" placeholder="Name"   id="input-name" class="form-control" />
                                      <input type="hidden" name="language_id" value="{{isset($language_id) ? $language_id : ''}}"  id="input-name" class="form-control" />
                                  </div>
                              </div>
                              ...
                              <div class="form-group required">
                                  ...
                                  <div class="col-sm-10">
                                      <input type="text" name="code" value="{{isset($code) ? $code : ''}}" placeholder="Code"   id="input-code" class="form-control" />
                                      </div>
                              </div>
                              <div class="form-group required">
                                  ...
                                  <div class="col-sm-3">
                                      <a href="" id="thumb-image" data-toggle="image" >
                                          <img src="{{ isset($image) ? asset('/view/image/' . $image ) : asset('/view/image/no-image.jpg')}}" alt="" title="" data-placeholder="<?php //echo $placeholder; ?>" /></a>
                              ...
                              <div class="form-group required">
                                  ...
                                  <div class="col-sm-10">
                                      <label class="radio-inline">
                                          <input type="radio" name="status" value="1" {{old('status' ,$status ??'')==1 ? 'checked' : '' }} />
                                          {{'Enabled'}}
                                      </label>
                                      <label class="radio-inline">
                                          <input type="radio" name="status" value="0" {{old('status' ,$status ??'')==0 ? 'checked' : '' }}/>
                                          {{'Disabled'}}
                              ...
                              <div class="form-group required">
                                  ...
                                  <div class="col-sm-10">
                                      <label class="radio-inline">
                                          <input type="radio" name="Directory" value="ltr" {{old('Directory' ,$directory ??'')==='ltr' ? 'checked' : '' }} />
                                          {{'Left_to_Right'}}
                                      </label>
                                      <label class="radio-inline">
                                          <input type="radio" name="Directory" value="rtl" {{old('Directory' ,$directory ??'')==='rtl' ? 'checked' : '' }}/>
                                          {{'Right_to_Left'}}
                              ...
                              <div class="form-group required">
                                  ...
                                  <div class="col-sm-10">
                                      <input type="text" name="sort" value="{{$sort ?? ''}}" placeholder="Sort Order"   id="input-code" class="form-control" />
                                  </div>
                              </div>
                          </div>
                  ...
                  <div class="row">
                      <div class="col-lg-3 col-md-3 col-sm-12">
                          <div class="list-group">
                              ...
                              <div class="list-group-item">
                                  <select name="store_id" class="form-control">
                                      ...
                                      <option value="1">{{ 'default12' }}</option>
                          ...
                          <div class="list-group">
                              <div class="list-group-item">
                                  <h4 class="list-group-item-heading"><i class="fa fa-file-text"></i>{{ ' Files' }}</h4>
                              </div>
                              <div id="path"></div>
                          </div>
                      </div>
      ...
      @push('lang_script')
         ...
         $('select[name="store_id"]').on('change', function(e) {
              $.ajax({
                  url: "{{url('admin/languages/path')}}"+ '/{{$code ?? 'templang'}}',
                  dataType: 'json',
                  beforeSend: function() {
...
*/
          $('#path').on('click', 'a.file', function(e) {
              ...
              if (!$('#tab-' + tab_id).length) {
                  $.ajax({
                      url: "{{url('admin/languages/getcontent')}}" +'/{{$code ?? 'templang'}}' + '/' +$(node).attr('href') ,
                      dataType: 'json',
                      beforeSend: function() {
                          $(node).find('i').removeClass('fa-arrow-right');
                          $(node).find('i').addClass('fa-circle-o-notch fa-spin');
                      ...
                      success: function(json) {
                          if (json['code']) {
                              ...
                              html += '    <button type="button" data-toggle="tooltip" data-original-title="only Save this file (' + $(node).attr('href') + ') " data-loading-text="loading" class="btn btn-primary"><i class="fa fa-floppy-o"></i> {{ 'Save this file' }}</button>';
                              html += '  </div>';
                              html += '</div>';
          ...
          $('#code .tab-content').on('click', '.btn-primary', function(e) {
              ...
              $.ajax({
                  url: "{{url('admin/languages/save')}}" +'/{{$code ?? 'templang'}}/'+$('#code .tab-content .active input[name="path"]').val(),
                  type: 'post',
