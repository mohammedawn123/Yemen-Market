...
<div class="well">
          <div class="row">
             ...
             <div class="form-group" >
                <label class="control-label" for="global_filter"> {{trans('user.label_search')}} </label>
               <input type="text" name="global_filter"  placeholder="{{trans('user.label_search')}} "  class="global_filter form-control" id="global_filter" />

            </div>
                 <div class="col-sm-4">
             <div class="form-group"  data-column="0">
                <label class="control-label" for="input-id"> {{trans('user.label_roles')}} </label>
               <input type="text" name="filter_id"  placeholder="{{trans('user.entry_roles')}}"  class="column_filter form-control" id="col0_filter" />
             </div>
             <div class="form-group" data-column="3">
                <label class="control-label" for="input-sort"> {{trans('user.label_permissions')}} </label>
                 <input type="text" name="filter_sort"   placeholder="{{trans('user.entry_permissions')}} "   class="column_filter form-control" id="col3_filter" />
              </div>
           ...
           <div class="col-sm-4">
                ...
                <label class="control-label" for="input-name">{{trans('user.label_username')}} </label>
                <input type="text" name="filter_name"   placeholder="{{trans('user.entry_username')}}"   class="column_filter form-control" id="col2_filter"/>
            </div>
            ...
            <table class="table table-bordered table-hover display"  id="languages_datatable">
              <thead>
                <tr>
                    ...
                    <td  class="text-center">{{trans('language.column_flag')}}</td>
                     <td  class="text-center">{{trans('language.column_name')}}</td>
                     <td  class="text-center">{{trans('language.column_code')}}</td>
                     <td  class="text-center">{{trans('language.column_status')}}</td>
                     <td  class="text-center">{{trans('language.column_directory')}} </td>
                     <td  class="text-center">{{trans('language.column_sort')}} </td>
                     <td>{{trans('language.column_action')}}</td>
                </tr>
              </thead>
              <tbody>
              @if(isset($languages))
                  @foreach ($languages as  $row)
                      <tr>
                          <td style="width: 1px; padding: 0px;" class="text-center">
                               <input type="checkbox" class="grid-row-checkbox"   data-id={{$row['language_id']}}>


                          </td>
                          <td  class="text-center">    <img src="{{asset('/view/image/' . $row['image'] )}}" alt="" title="" data-placeholder="<?php //echo $placeholder; ?>" /></td>
